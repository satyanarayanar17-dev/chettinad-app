...
@Composable
fun NurseTriageQueueScreen(
    uiState: NurseQueueUiState,
    onLoadQueue: () -> Unit,
            ...
            if (uiState.isLoading && uiState.queue.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(uiState.queue) { patient ->
                        NurseQueueCard(patient = patient, onClick = { onPatientClick(patient.id) })
                        HorizontalDivider(
...
@Composable
fun NurseQueueCard(patient: Patient, onClick: () -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
